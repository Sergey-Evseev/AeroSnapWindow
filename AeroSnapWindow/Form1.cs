...
namespace AeroSnapWindow
{
    public partial class Form1 : Form
    {
        ...
        //keeps form size when it is minimized and restored
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            base.WndProc(ref m);
        } // end of protected override void WndProc(ref Message m) 
            ...
            switch (this.WindowState)//в зависимости от состояния формы
            {
                ...
                case FormWindowState.Normal:
                    if (this.Padding.Top != borderSize)
                        //окно в нормальнос сост. приводится к нач. значениям = 2
        ...
        //EVENT METHODS - МЕТОДЫ ОБРАБОТЧИКИ СОБЫТИЙ
        ...
        {   //если окно было в нормальном состоянии            
            if (this.WindowState == FormWindowState.Normal)
            {
                ...
                // a form where controls can be placed
                formSize = this.ClientSize; //перед раскрытием формы запомнить ее
                //размер (клиентская область без тайтл-бара) в нормальном состоянии
                //...то развернуть его
